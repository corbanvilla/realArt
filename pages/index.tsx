...
export default function Home({ paintings }: InferGetStaticPropsType<typeof getServerSideProps>) {
    ...
    return (
        ...
        {/* Main Grid */}
            ...
            <div className="flex flex-row min-h-[70vh] items-center place-content-center gap-x-16 lg:gap-x-8 xl:gap-x-16">
                ...
                {/* Left Card */}
                <div style={{ zIndex: overlayLeft }} onClick={enableOverlayLeft} className="flip-card cursor-pointer h-96 w-96 min-h-[380px] min-w-[380px]">
                    <div className={"flip-card-inner" + (overlayLeft == 1 ? " do-flip" : "")}>
                        {/* Front Side */}
                        ...
                        <div className="flip-card-back bg-gray-200">
                            <div className="p-8">
                                <p className="text-4xl pb-6 text-center">Real Art!</p>
                                <p className="px-4">This painting is in the Louvre Abu Dhabi!</p><br></br>
                                <div className="mx-2 bg-gray-300 p-6">
                                    <p>Some silly boyz really did go down by the river</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {/* Right Card */}
                <div style={{ zIndex: overlayRight }} onClick={enableOverlayRight} className="flip-card cursor-pointer h-96 w-96 min-h-[380px] min-w-[380px]">
                    <div className={"flip-card-inner" + (overlayRight == 1 ? " do-flip" : "")}>
                        {/* Front Side */}
                        <div className="flip-card-front border-8 border-gray-200">
                            <Image
                            fill
                        ...
                        <div className="flip-card-back bg-gray-200">
                            <div className="p-8">
                                <p className="text-4xl pb-6 text-center">AI Generated</p>
                                <p className="px-4">This painting was generated with the following prompt:</p><br></br>
                                <div className="mx-2 bg-gray-300 p-6">
                                    <p>Some silly boyz down by the river &#8249; &#8250;</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {/* Overlays */}
            {/* Dark Overlay */}
            <div onClick={resetOverlay} hidden={overlayHidden} className="p-20 fixed top-0 left-0 bottom-0 right-0 w-full h-screen bg-black bg-opacity-50"></div>
            {/* Next button */}
            <div onClick={nextPainting} style={showNextButton ? {} : { display: "none" }} className="fixed top-0 right-0 w-32 lg:w-32 xl:w-64 h-screen z-20 bg-gray-100">
            ...
            {/* Progress Bar */}
            <div style={showNextButton ? {} : { display: "none" }} className="fixed bottom-0 left-0 w-screen h-10 bg-red-300 z-10">
                <div
                    className="h-full bg-green-500"
                    style={{ width: `${calcVotePercentage()}%` }}
                >
                    <p className="text-white px-4 py-2">{painting.real.votes} out of {painting.real.votes + painting.fake.votes} votes were correct!</p>
                </div>
            </div>
...
export async function getServerSideProps() {
  ...
  // Shuffle order
    const paintings = res.map(value => ({ value, sort: Math.random() }))
